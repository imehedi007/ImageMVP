...
export async function findLatestOtpRequest(phone: string) {
  ...
  const [rows] = await db.query<OtpRow[]>(
    `SELECT id, phone, otp_hash, expires_at, attempt_count, verified_at, created_at
     FROM otp_requests
     WHERE phone = ?
     ORDER BY id DESC
     LIMIT 1`,
...
export async function incrementOtpAttempt(id: number) {
  await ensureDatabaseSchema();
  const db = getDbPool();
  await db.query(`UPDATE otp_requests SET attempt_count = attempt_count + 1 WHERE id = ?`, [id]);
}

export async function markOtpVerified(id: number) {
  await ensureDatabaseSchema();
  const db = getDbPool();
  await db.query(`UPDATE otp_requests SET verified_at = NOW() WHERE id = ?`, [id]);
}
...
export async function completeGenerationLog(id: number) {
  await ensureDatabaseSchema();
  const db = getDbPool();
  await db.query(`UPDATE generation_logs SET status = 'completed', error_message = NULL WHERE id = ?`, [id]);
}

export async function failGenerationLog(id: number, message: string) {
  await ensureDatabaseSchema();
  const db = getDbPool();
  await db.query(`UPDATE generation_logs SET status = 'failed', error_message = ? WHERE id = ?`, [message, id]);
}
...
export async function getDashboardOverview(): Promise<DashboardOverview> {
  ...
  const [[usersRow]] = await db.query<RowDataPacket[]>(
    `SELECT
      COUNT(*) AS totalUsers,
  ...
  const [[generationRow]] = await db.query<RowDataPacket[]>(
    `SELECT
...
export async function searchUsersByPhone(phoneQuery: string) {
  ...
  const [rows] = await db.query<DashboardSearchRow[]>(
    `SELECT
      u.id AS userId,
      u.name,
      u.phone,
